...
import { Loader2, Plus, Wrench, CheckCircle2, Clock, Package, AlertTriangle } from "lucide-react";
...
export default function ContaRMA() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toast.success("Pedido submetido. Entraremos em contacto brevemente.");
    setOpen(false);
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Preencha os dados do equipamento. Entraremos em contacto em 24-48h.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input value={form.serial_number} onChange={e => setForm({...form, serial_number: e.target.value})} placeholder="Opcional" />
              </div>
              <div className="space-y-2">
                <Label>Nº fatura</Label>
                <Input value={form.invoice_number} onChange={e => setForm({...form, invoice_number: e.target.value})} placeholder="Opcional" />
              </div>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.reason} onChange={e => setForm({...form, reason: e.target.value})}
                placeholder="Ex: não liga, ecrã partido..." required />
            </div>
            <div className="space-y-2">
              <Label>Descrição detalhada</Label>
              <Textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})}
                placeholder="Descreva o problema com o máximo de detalhe..." rows={3} />
            </div>
